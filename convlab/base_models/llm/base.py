...
class LLM:
   ...
   def chat(self, message, **kwargs):
      ...
      response = self.model.chat(
         messages, **{**self.generation_kwargs, **kwargs})
   ...
   def generate(self, prompt, **kwargs):
      ...
      response = self.model.generate(
         self.system_instruction, prompt, **{**self.generation_kwargs, **kwargs})
      return response
...
class ChatGLM2(BaseLLM):
   ...
   def chat(self, messages, **kwargs):
      messages = [{"role": messages[1]["role"], "content": messages[0]
                ["content"] + "\n\n" + messages[1]["content"]}] + messages[2:]
      history = [(query['content'], response['content'])
               for query, response in zip(messages[::2], messages[1::2])]
      response, history = self.model.chat(
         self.tokenizer, messages[-1]['content'], history=history, **kwargs)
      print(history)
      return response

   def generate(self, system_instruction, prompt, **kwargs):
      response, history = self.model.chat(
         self.tokenizer, system_instruction+"\n\n"+prompt, history=[], **kwargs)
      print(history)
      return response
...
if __name__ == '__main__':
   ...
   model = LLM('huggingface', 'meta-llama/Llama-2-7b-chat-hf')
   ...
   model.set_system_instruction(
      "Let's play a game. I'll start my sentence with 'fortunately' and you should go on but start the sentence with 'unfortunately'")
